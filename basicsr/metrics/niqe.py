...
def calculate_niqe(img, crop_border, input_order='HWC', convert_to='y'):
    ...
    # we use the official params estimated from the pristine dataset.
    niqe_pris_params = np.load('/media/a204/742CCF0E2CCEC9F6/chenhj/RetinexWT/RetinexWT-master/basicsr/metrics/niqe_pris_params.npz')
...
def calculate_niqe_for_dataset(folder_path, crop_border=0, input_order='HWC', convert_to='y'):
    # 加载 NIQE 的预训练参数
    niqe_pris_params = np.load('/media/a204/742CCF0E2CCEC9F6/chenhj/RetinexWT/RetinexWT-master/basicsr/metrics/niqe_pris_params.npz')
    mu_pris_param = niqe_pris_params['mu_pris_param']
    cov_pris_param = niqe_pris_params['cov_pris_param']
    ...
    for img_name in os.listdir(folder_path):
        ...
        try:
            ...
            # 计算 NIQE
            niqe_score = niqe(img, mu_pris_param, cov_pris_param, gaussian_window).item()  # 转换为标量
            results[img_name] = niqe_score

            # 累计 NIQE 值并增加计数
            total_niqe += niqe_score
            count += 1

            print(f"Image: {img_name}, NIQE: {niqe_score:.3f}")
        except Exception as e:
            print(f"Error processing {img_name}: {e}")
    # 计算平均值
    if count > 0:
        average_niqe = total_niqe / count
        print(f"\nAverage NIQE: {average_niqe:.3f}")
    else:
        print("\nNo valid images found for NIQE calculation.")
    return results


# 示例：处理某文件夹下的图像
folder_path = '/media/a204/742CCF0E2CCEC9F6/chenhj/RetinexWT/RetinexWT-master/results/mef/MSE/best_psnr_25'  # 替换为测试集路径
niqe_results = calculate_niqe_for_dataset(folder_path)
