...
class DConv3(nn.Module):
    def __init__(self, f_number, padding_mode='reflect') -> None:
        super().__init__()
        self.dconv = nn.Conv2d(f_number, f_number, kernel_size=3, padding=3, groups=f_number, padding_mode=padding_mode)
...
class CID(nn.Module):
    def __init__(self, f_number, padding_mode) -> None:
        super().__init__()
        self.channel_independent = DConv3(f_number, padding_mode)
...
class GFM(nn.Module):
    def __init__(self, in_channels, feature_num=2, bias=True, padding_mode='reflect', **kwargs) -> None:
        ...
        self.conv = nn.Conv2d(in_channels, hidden_features, 1, 1, 0, bias=True) # cc
    ...
    def forward(self, *inp_feats):
        ...
        x = self.conv(self.mlp(x + shortcut)) # cc
        return x  # self.mlp(x + shortcut)
